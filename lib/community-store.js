...
function toPublicPost(post, viewerToken = '') {
  return {
    ...
    likedByMe: Boolean(viewerToken && post.likedTokens.includes(viewerToken)),
    canEdit: Boolean(viewerToken && post.authorToken === viewerToken)
  };
}
...
export async function updateCommunityPost(id, input, authorToken, viewerToken = authorToken) {
  ...
  if (!authorToken || target.authorToken !== authorToken) throw new Error('forbidden');
...
export async function toggleCommunityPostLike(id, viewerToken) {
  ...
  const liked = target.likedTokens.includes(viewerToken);
  const likedTokens = liked
    ? target.likedTokens.filter((token) => token !== viewerToken)
    : [...target.likedTokens, viewerToken];
  const next = {
    ...target,
    likedTokens,
