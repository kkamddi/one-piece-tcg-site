...
const DEFAULT_INITIAL_OBSERVATION_DAYS = 10;
...
export function buildInitialPriceFormationSeries(series = [], options = {}) {
  const initialObservationDays = Math.max(
    1,
    Number(options.initialObservationDays || DEFAULT_INITIAL_OBSERVATION_DAYS)
  );
  ...
  for (const point of series) {
    ...
    pointsByDate.set(date, { ...point, date, price: Math.round(price) });
  }

  const observed = [...pointsByDate.values()].sort((a, b) => a.date.localeCompare(b.date));
  if (observed.length < initialObservationDays) {
    return {
      series: [],
      firstObservedDate: observed[0]?.date || '',
      firstObservedPrice: Number(observed[0]?.price || 0),
      baselineDate: '',
      baselinePrice: 0,
      observationCount: observed.length,
      requiredObservationCount: initialObservationDays,
      reason: 'insufficient-initial-observations'
    };
  }

  const initialWindow = observed.slice(0, initialObservationDays);
  const baselineDate = initialWindow.at(-1).date;
  ...
  return {
    ...
    firstObservedDate: observed[0].date,
    firstObservedPrice: observed[0].price,
    baselineDate,
    baselinePrice,
...
export function buildEqualWeightedMarketIndex(indexConfig, rows = [], options = {}) {
  ...
  const baselineComponents = rawComponents.map((component) => ({
    ...component,
    baseline: buildInitialPriceFormationSeries(component.rawSeries, options)
  }));
