...
import specialPromoCards from '../src/data/special-promo-cards.js';
...
async function readBundledCards() {
  bundledCardsPromise ??= import('../src/data/cards.json', { with: { type: 'json' } })
    .then((module) => mergeSpecialPromoCards(module.default));
  return bundledCardsPromise;
}

function mergeSpecialPromoCards(cards = []) {
  const merged = new Map(cards.map((card) => [card.id, card]));
  specialPromoCards.forEach((card) => merged.set(card.id, card));
...
export async function readCards(filters = {}) {
  ...
  if (snapshotCards?.every(Array.isArray)) {
    const flattenedCards = mergeSpecialPromoCards(snapshotCards.flat());
  ...
  if (d1Cards) return mergeSpecialPromoCards(d1Cards);
...
export async function readCardById(id) {
  ...
  const specialPromoCard = specialPromoCards.find((item) => item.id === id);
  if (specialPromoCard) return specialPromoCard;
