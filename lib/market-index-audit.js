...
export function auditEqualWeightedMarketIndex(built = {}, tolerance = 0.0002) {
  ...
  const admissions = Array.isArray(built.admissions) ? built.admissions : [];
  ...
  for (let index = 0; index < indexPoints.length; index += 1) {
    ...
    const divisor = Number(point?.divisor || 0);
    const expected = divisor > 0 ? componentSum / divisor : 0;
    if (!values.length
    ...
    if (index > 0) {
      const change = percentChange(Number(point?.value || 0), Number(indexPoints[index - 1]?.value || 0));
      if (Math.abs(change) > 20 + tolerance) {
        dailyMoveViolations.push({ date: point?.date || '', change: Number(change.toFixed(4)) });
      }
    }
  }

  const admissionViolations = admissions.filter((admission) => (
    Math.abs(Number(admission?.valueAfterAdmission || 0) - Number(admission?.valueBeforeAdmission || 0)) > tolerance
  ));
