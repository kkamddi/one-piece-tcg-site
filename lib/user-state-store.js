...
function normalizeMarketItems(input = {}) {
  ...
  return Object.fromEntries(Object.entries(input).map(([key, item]) => {
    ...
    return [safeKey, {
      ...
      priceCurrency: ['JPY', 'USD'].includes(String(item?.priceCurrency || '').toUpperCase())
        ? String(item.priceCurrency).toUpperCase()
        : 'JPY',
      fallbackPriceUsd: Number(item?.fallbackPriceUsd ?? 0) || 0
    }];
  }).filter(Boolean));
}

function normalizePortfolioLots(input = {}) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) return {};
  return Object.fromEntries(Object.entries(input).map(([key, lots]) => {
    const safeKey = String(key ?? '').trim();
    if (!safeKey || !Array.isArray(lots)) return null;
    const normalizedLots = lots.map((lot, index) => {
      const mode = ['manual', 'estimate', 'later'].includes(String(lot?.mode || '').toLowerCase())
        ? String(lot.mode).toLowerCase()
        : 'later';
      const currency = ['KRW', 'JPY', 'USD'].includes(String(lot?.originalCurrency || '').toUpperCase())
        ? String(lot.originalCurrency).toUpperCase()
        : 'KRW';
      const purchaseDate = /^\d{4}-\d{2}-\d{2}$/.test(String(lot?.purchaseDate || ''))
        ? String(lot.purchaseDate)
        : '';
      const createdAt = String(lot?.createdAt || '').trim();
      return {
        id: String(lot?.id || `lot-${index}-${purchaseDate || 'undated'}`).trim(),
        mode,
        quantity: Math.min(9999, Math.max(1, Math.round(Number(lot?.quantity || 1) || 1))),
        purchaseDate,
        originalCurrency: currency,
        originalUnitPrice: Math.max(0, Number(lot?.originalUnitPrice || 0) || 0),
        unitPriceJpy: Math.max(0, Number(lot?.unitPriceJpy || 0) || 0),
        referenceDate: String(lot?.referenceDate || '').trim(),
        referenceSource: ['listing', 'trade'].includes(String(lot?.referenceSource || '').toLowerCase())
          ? String(lot.referenceSource).toLowerCase()
          : '',
        createdAt,
        updatedAt: String(lot?.updatedAt || createdAt).trim()
      };
    }).filter((lot) => lot.id);
    return normalizedLots.length ? [safeKey, normalizedLots] : null;
  }).filter(Boolean));
}
...
function normalizeState(input = {}) {
  return {
    ...
    portfolioLots: normalizePortfolioLots(input.portfolioLots),
    portfolioMigratedAt: input.portfolioMigratedAt ? String(input.portfolioMigratedAt) : null,
...
function hasUserData(state = {}) {
  return normalizeStringArray(state.ownedCardIds).length > 0
    ...
    || Object.keys(state.portfolioLots || {}).length > 0
