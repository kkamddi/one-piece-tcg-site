...
import { extractCardCodes, getCameraCrop, getScanVariants, MAX_SCAN_BYTES, validateScanFile } from '../src/lib/card-scan.js';
...
async function ocrWithWorker(createWorker) {
  ...
  return vm.runInNewContext(`(${source.slice(fn.start, fn.end)})`, {
    createWorker, workerPath: 'test-worker', extractCardCodes, DOMException,
...
test('OCR falls back to the full photo and always terminates its worker', async () => {
  ...
  const scan = await ocrWithWorker(async () => ({
    setParameters: async () => {},
    recognize: async () => ({ data: { text: ++calls < 3 ? '5000' : 'OP01-120' } }),
