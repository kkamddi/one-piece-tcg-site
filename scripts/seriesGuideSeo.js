...
function createSeo(series, cardCount, cardSummary, japanese = false) {
  ...
  if (japanese) {
    return {
      ...
      sections: [
        { heading: 'レアリティ構成', paragraphs: [`基本カード番号基準では${raritySummary || '登録情報を確認中です'}。同じカード番号のパラレル画像は重複集計していません。`] },
        { heading: '収録カード例', items: cardExamples },
  ...
  return {
    ...
    sections: [
      { heading: '레어도 구성', paragraphs: [`기본 카드번호 기준 ${raritySummary || '등록 정보 확인 중'}입니다. 같은 카드번호의 패러렐 이미지는 중복 집계하지 않았습니다.`] },
      { heading: '수록 카드 예시', items: cardExamples },
...
export function getSeriesGuideEntries({ japanese = false } = {}) {
  return seriesData
    .map((series) => {
      ...
      return { pathname: japanese ? `/jp${basePath}` : basePath, seo: createSeo(series, cardCount, cardSummary, japanese) };
    })
    .filter(Boolean);
}
