...
const COMMUNITY_STORAGE_KEY = 'one-piece-tcg-community-posts';
const COMMUNITY_NICKNAME_KEY = 'one-piece-tcg-community-nickname';
const COMMUNITY_LIKES_KEY = 'one-piece-tcg-community-liked-posts';
...
export default function App() {
  ...
  const [likedCommunityPostIds, setLikedCommunityPostIds] = useState([]);
  ...
  useEffect(() => {
    ...
    try {
      const savedPosts = JSON.parse(window.localStorage.getItem(COMMUNITY_STORAGE_KEY) ?? '[]');
      if (Array.isArray(savedPosts)) setCommunityPosts(savedPosts);
    } catch {}

    try {
      const savedLikedPosts = JSON.parse(window.localStorage.getItem(COMMUNITY_LIKES_KEY) ?? '[]');
      if (Array.isArray(savedLikedPosts)) setLikedCommunityPostIds(savedLikedPosts);
    } catch {}
  ...
  useEffect(() => {
    window.localStorage.setItem(COMMUNITY_STORAGE_KEY, JSON.stringify(communityPosts));
  }, [communityPosts]);
  ...
  useEffect(() => {
    window.localStorage.setItem(COMMUNITY_LIKES_KEY, JSON.stringify(likedCommunityPostIds));
  ...
  function openCommunityPost(post) {
    setCommunityPosts((prev) => prev.map((item) => item.id === post.id ? { ...item, views: (item.views ?? 0) + 1 } : item));
    setSelectedCommunityPost({ ...post, views: (post.views ?? 0) + 1 });
  }

  function toggleCommunityLike(postId) {
    const liked = likedCommunityPostIds.includes(postId);
    setLikedCommunityPostIds((prev) => liked ? prev.filter((id) => id !== postId) : [...prev, postId]);
    setCommunityPosts((prev) => prev.map((post) => post.id === postId
      ? { ...post, likes: Math.max(0, (post.likes ?? 0) + (liked ? -1 : 1)) }
      : post));
    if (selectedCommunityPost?.id === postId) {
      setSelectedCommunityPost((prev) => prev ? { ...prev, likes: Math.max(0, (prev.likes ?? 0) + (liked ? -1 : 1)) } : prev);
    }
  }
      ...
      <div className={`min-h-screen ${shellClass}`}>
        <div className="mx-auto max-w-[1880px] px-3 py-4 sm:px-6 sm:py-5 lg:px-8">
          ...
          <div className={`grid gap-5 ${viewMode === 'home' || viewMode === 'deck' || viewMode === 'shops' || viewMode === 'community' ? 'xl:grid-cols-1' : 'xl:grid-cols-[280px_minmax(0,1fr)]'}`}>
            {viewMode === 'home' || viewMode === 'deck' || viewMode === 'shops' || viewMode === 'community' ? null : <aside className={`border ${panelClass} rounded-2xl p-3`}>
                      ...
                      <article key={post.id} className={`border ${cardClass} rounded-2xl p-4 shadow-sm sm:p-5`}>
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button type="button" onClick={() => startEditCommunityPost(post)} className={`rounded-full border px-3 py-1.5 text-xs font-bold ${subtleClass}`}>수정</button>
                            <button type="button" onClick={() => deleteCommunityPost(post.id)} className="rounded-full bg-red-500 px-3 py-1.5 text-xs font-bold text-white">삭제</button>
                          </div>
                        </div>
            ...
            <div className={`w-full max-w-2xl rounded-2xl border shadow-2xl ${isDark ? 'border-[#363636] bg-[#202020]' : 'border-[#e4d7c7] bg-[#fffdf9]'}`} onClick={(event) => event.stopPropagation()}>
              ...
              <form onSubmit={submitCommunityPost} className="space-y-3 p-5">
                <label className="block">
                  ...
                  <input value={communityNickname} onChange={(event) => setCommunityNickname(event.target.value)} placeholder="닉네임 입력" className={`w-full rounded-xl border px-4 py-3 text-sm outline-none ${subtleClass} ${isDark ? 'placeholder:text-stone-500' : 'placeholder:text-stone-400'} focus:border-[#c94d35]`} />
                </label>
                <label className="block">
                ...
                <label className="block">
                  ...
                  <textarea value={communityContent} onChange={(event) => setCommunityContent(event.target.value)} placeholder={communityBoard === 'showoff' ? '자랑 한마디 적기' : '자유롭게 글쓰기'} rows={6} className={`w-full rounded-xl border px-4 py-3 text-sm outline-none ${subtleClass} ${isDark ? 'placeholder:text-stone-500' : 'placeholder:text-stone-400'} focus:border-[#c94d35]`} />
                </label>
